        ...
        fetch("/api/alerts", { cache: "no-store" })
            ...
            .then((data: any[]) => {
                ...
                for (const al of (Array.isArray(data) ? data : [])) {
                    ...
                    const camName = al?.camera?.name ?? al?.cam_name ?? al?.camera_name ?? (camId != null ? `Camera ${camId}` : null);
                    const locName =
                        al?.camera?.location?.name ??
                        al?.location?.name ??
                        al?.loc_name ??
                        (typeof al?.camera?.location === "string" ? al?.camera?.location : null);
                    if (camId == null || !camName) continue;
                    const key = String(camId);
                    if (!map.has(key)) map.set(key, { id: camId, name: camName, location: locName ?? undefined });
                }
                const list = Array.from(map.values()).sort((a, b) => String(a.name).localeCompare(String(b.name)));
                setCams(list);
            })
            .catch((e) => mounted && setCamErr(e?.message || "Failed to load cameras"))
            .finally(() => mounted && setCamLoading(false));
        return () => { mounted = false; };
    }, []);

    const sevOpts = useMemo(() => ["All", ...severityOptions], [severityOptions]);
    const statOpts = useMemo(() => ["All", ...statusOptions], [statusOptions]);
    const evOpts = useMemo(() => ["All", ...eventOptions], [eventOptions]);

    const hasAny =
        !!severityValue || !!statusValue || !!eventValue || !!cameraValue || !!fromValue || !!toValue;

    // ---------- RETURN: ฟิลเตอร์แถวบน + date บล็อกล่าง ----------
    return (
        <div className="w-full">
            {/* แถวเดียวทั้งหมดบนจอใหญ่; จอเล็กจะ wrap อัตโนมัติ */}
            <div className="grid gap-2 grid-cols-1 min-[420px]:grid-cols-2 sm:grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] w-full items-stretch">
                ...
                <div className="w-full">
                    <Field
                        ...
                        value={severityValue}
                        onChange={(v) => setParam("severity", v === "All" ? null : v.toLowerCase())}
                    >
                ...
                <div className="w-full">
                    <Field
                        ...
                        value={statusValue}
                        onChange={(v) => setParam("status", v === "All" ? null : v.toLowerCase())}
                    >
                ...
                {/* Reset (auto กว้างเท่าที่จำเป็น ชิดขวา) */}
